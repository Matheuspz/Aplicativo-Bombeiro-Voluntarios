<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Tela Inicial Admin</title>
    <link rel="stylesheet" href="../css/telaInicial.css">
</head>
<body>
    <header>
        <img src="../img/logo_grande.png" alt="logoNOAR">
        <h1>Associação Humanitária de Busca Salvamento Aéreo e Resgate</h1>
        <h1 class="cellphone">AHBSAR</h1>
        <div class="menu">
            <div class="menu_burguer"></div>
            <nav class="hidden_nav">
                <ul>
                    <li><a href="../index.php">SAIR</a></li>
                </ul>
            </nav>
        </div>
    </header>
    <main>
        <a href="telaOcorrencia1.php"><button>Nova ocorrencia</button></a>        <hr>
        <a href="historicoDeOcorrencia.php"><button>Histórico de formulários</button></a>
        <hr>
        <a href="crudAdmin.php"><button>Usuarios</button></a>
    </main>
</body>
<script src="../js/telaInicial.js"></script>
</html>